...
public class WristController : MonoBehaviour
{
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        {
            //do not do anything if we collide with the other object representing the other wrist OR
            //do not do anything if we collide with the walls.
        }
        else
        { //however, if we collide with a ball that has the same tag than the target balls that we must collect, we will win points.
            //other.gameObject.SetActive(false);
            Destroy(other.gameObject);
